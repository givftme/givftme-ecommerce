-- Migration 029: Owner-chosen wishlist cover colour
--
-- A wishlist cover is one of a fixed palette of brand gradients, stored as its
-- key. NULL means the default cover. The keys must match
-- WISHLIST_COVER_KEYS in lib/wishlist/covers.ts.

ALTER TABLE public.wishlists
  ADD COLUMN IF NOT EXISTS cover_color text;

ALTER TABLE public.wishlists
  DROP CONSTRAINT IF EXISTS wishlists_cover_color_check;

ALTER TABLE public.wishlists
  ADD CONSTRAINT wishlists_cover_color_check CHECK (
    cover_color IS NULL
    OR cover_color IN ('ruby', 'midnight', 'sunset', 'forest', 'ocean', 'plum', 'blush')
  );

-- Expose the cover to people viewing a shared wishlist. This is the
-- migration 013 resolver unchanged, except that it now selects and returns
-- wishlist.cover_color. Access rules are untouched.
  ...
  RETURN jsonb_build_object(
    ...
    'wishlist',
      jsonb_build_object(
        ...
        'cover_color', wishlist_record.cover_color,
