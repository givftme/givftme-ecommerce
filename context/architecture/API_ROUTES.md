...
**Method:** GET. **Auth:** none. **Purpose:** fetches the next paginated batch of currently-active flash sale products for `/flash-sale`'s `ProductExplorer` load-more, mirroring `/api/shop/products` but against `FLASH_SALE_PRODUCTS_QUERY`/`FLASH_SALE_PRODUCTS_COUNT_QUERY` instead of the full catalog.
**Query params:** `offset` (default 0), `limit` (default 16, max 48).
...
**Response (new order):** `{ order_id, payment_link, price_changed, price_changes }` — `price_changed` is `true` when any item's server-computed price (via `getActivePrice`, `lib/flutterwave/getActivePrice.ts`) differs from the client's submitted `display_price` (e.g. a flash sale ended between page load and submit); `price_changes` lists `{ title, old_price, new_price }` per affected item. The order is always created at the correct server price regardless — `CheckoutForm.tsx` shows a confirmation dialog before redirecting to the Flutterwave payment link when this flag is set, but the order itself is not blocked on that confirmation.
**Response (existing/replayed order):** `{ order_id, payment_link }` only — an idempotency-key replay or an already-resolved order never recomputes pricing, since `order_items.unit_price` is fixed at creation time.
**Flash sale grace period:** `getActivePrice` still honors `salePrice` for up to 5 minutes after `saleEndTime` (see 18-FLASH-SALES.md) — a pure time-based window, not an order-age check, since order creation and price computation happen in this same call.
**Critical:** fetches prices from Sanity using `CART_PRICES_QUERY` and never trusts client-submitted prices for the charge. Creates `orders.status = 'pending_payment'` and `order_items.unit_price` before contacting Flutterwave. If Flutterwave initiation fails, returns 502 while leaving the order retryable.
...
**Execution limit:** `maxDuration = 300` seconds, exported as a literal so Next.js can statically analyze the route configuration.
