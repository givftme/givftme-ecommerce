...
Append-only log, auto-populated by the `on_order_status_changed` trigger whenever `orders.status` changes. Never insert into this table directly from application code — let the trigger do it. Existed live with no migration file backing it until migration 019 (`16-ORDER-TRACKING.md`), the first to formally own its schema (`CREATE TABLE IF NOT EXISTS` + defensive `ADD COLUMN IF NOT EXISTS`, same pattern migrations 015/016 used for `important_dates`/`thank_you_messages`). That migration adds `customer_notified` (set by `/api/orders/notify` after a successful send), `notes` (copied from `orders.status_change_notes` by the trigger), and — beyond the spec's literal `Database Changes` SQL, but required to implement its own Edge Case #4 and to give the notify cron the same concurrency guard every other cron in this repo already has — `retry_count`, `permanently_failed`, and `claimed_at`. RLS (new — no customer read path existed before this feature) allows a buyer to `SELECT` history rows for their own orders. `validate_order_status_transition` (also migration 019, a `BEFORE UPDATE` trigger on `orders`) enforces the spec's literal transition map at the DB level regardless of writer, blocking backward moves (e.g. `shipped` → `forwarded`) and skipped steps (e.g. `forwarded` → `delivered`).
...
### `reviews`
Added by migration 020 (`17-REVIEWS.md`) — the first table in this repo's schema history that never existed live beforehand, so it's a plain `CREATE TABLE`, not the defensive `CREATE IF NOT EXISTS` pattern migrations 015/016/019 needed. Columns: `id`, `user_id` (FK to `public.users`, cascades on delete), `catalog_product_id` (Sanity `_id`, plain TEXT — no FK, since Sanity documents aren't in this database), `rating` (1–5, checked), `body` (optional, ≤1000 chars, checked), `created_at`, `updated_at` (auto-maintained by a new `update_reviews_updated_at` trigger). `one_review_per_user_per_product` UNIQUE constraint on `(user_id, catalog_product_id)` enforces business rule #14 at the DB level. No `order_item_id` column — an earlier draft of this note assumed one would be needed to gate verified purchases, but the actual gate is a runtime join (`isVerifiedPurchaser` in `lib/reviews/server.ts`, checking `orders`/`order_items` for a `delivered` order containing the product) done in the `POST /api/reviews` route handler, not a stored FK; RLS only enforces "write your own row" (see `API_ROUTES.md`). Migration 020 also defines `update_updated_at_column()` itself — the spec assumed it already existed from migrations 001/002, but no migration file in this repo ever defined it or used an `updated_at` trigger before this one.
