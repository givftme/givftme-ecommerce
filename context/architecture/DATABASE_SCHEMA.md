...
### `orders`
Migration 025 adds `price_changes` (`jsonb`, non-null array, default `[]`). `gifvtme_create_checkout_order` snapshots mismatched item titles, client `display_price` as `old_price`, and server `unit_price` as `new_price` in this column during the same transaction as order/items creation. These values only describe the original price warning; `unit_price` remains the authoritative charge. Existing orders retain `[]` because their original displayed prices cannot be recovered. Apply migration 025 before deploying checkout code that selects this column.
Catalog-flow purchases only. Tracks the full lifecycle via `status`: `pending_payment` → `confirmed`/`payment_failed` → `under_review` → `forwarded` → `shipped` → `delivered` (or `cancelled`/`refunded` at various points). Carries Flutterwave transaction references, shipping details, and tracking info. `wishlist_item_id` is nullable — set when the order originated from a wishlist purchase, null for direct shop purchases; migration 019 gives its FK `ON DELETE SET NULL` (`16-ORDER-TRACKING.md` Edge Case #6 — a deleted wishlist item must not take order history with it). Existed live with no migration file backing it (like `thank_you_messages`/`important_dates` before their own migrations) until migration 017, which adds the one column needed for checkout idempotency: `idempotency_key` (nullable text, unique **per buyer** among non-null values via a partial unique index on `(buyer_id, idempotency_key)` — not globally unique, since the app only ever looks up a key alongside its owning `buyer_id`) — set from the client-generated `Idempotency-Key` header on `POST /api/checkout` so a retried/duplicated submission reuses the same order instead of creating a new one. Migration 018 adds `gifvtme_create_checkout_order(...)`, a security-invoker RPC function that inserts an `orders` row and its `order_items` together in one transaction — `POST /api/checkout` calls this instead of two separate inserts, so no request can ever observe an order without its items (the earlier two-insert version had a real race: a concurrent idempotent replay could find the order row and initiate payment before its items existed). That same migration also adds `payment_claimed_at` (nullable timestamptz), set at creation time and used as a `claimed_at`-style compare-and-set lock (same pattern as `reminders`/`thank_you_messages`) so two concurrent payment-initiation attempts for one order — a double-click retry, or two racing idempotent replays — can't each start an independent, separately-payable Flutterwave session for it. See `API_ROUTES.md`'s `/api/checkout` entry. Migration 019 (`16-ORDER-TRACKING.md`) adds `tracking_number`, `tracking_url`, `carrier_name`, `estimated_delivery` (set by Retool when status moves to `shipped`), and `status_change_notes` (an ops note Retool sets alongside a status change; copied into the new `order_status_history` row by the trigger below, then cleared).
...
Append-only log, auto-populated by the `on_order_status_changed` trigger whenever `orders.status` changes. Never insert into this table directly from application code — let the trigger do it. Columns: `id`, `order_id`, `status`, `changed_at`, `customer_notified` (set by `/api/orders/notify` after a successful send), `notes` (copied from `orders.status_change_notes` by the trigger), `retry_count`, `permanently_failed`, `claimed_at`. RLS allows a buyer to `SELECT` history rows for their own orders. `validate_order_status_transition` (migration 019, updated by migration 021, a `BEFORE UPDATE` trigger on `orders`) enforces the transition map at the DB level regardless of writer, blocking backward moves (e.g. `shipped` → `forwarded`) and skipped steps (e.g. `forwarded` → `delivered`).

**Schema history, if this ever needs revisiting** — this table predates this repo's migration history and went through several rounds of correction (019 → 022 → 023 → 024, all in `context/ROADMAP.md`'s dated entries): it originally existed live with an unrelated `old_status`/`new_status`/`changed_by`/`note`/`created_at` shape, not the `status`/`changed_at` migration 019 assumed when it tried to formally own the table; migration 022 added the missing `status`/`changed_at`; migration 023 then found `new_status` was `NOT NULL` with no default and had to populate it just to stop every transition from failing; and after `status` on already-written rows was twice observed changing on its own with no discoverable cause (every mechanism checked and ruled out — see ROADMAP.md's 2026-08-13 entry), migration 024 dropped the legacy `old_status`/`new_status`/`changed_by`/`note` columns entirely rather than continuing to carry two overlapping status models, restoring the trigger to the literal single-column design `16-ORDER-TRACKING.md` always specified.

### `thank_you_messages`
References either `purchase_id` (external flow) or `order_id` (catalog flow) — exactly one must be set, enforced by a check constraint. `type` is `auto` or `personal`. Existed live with no migration file backing it until migration 016 — that migration is the first to formally own its schema (`CREATE TABLE IF NOT EXISTS` + defensive `ADD COLUMN IF NOT EXISTS` patches, same pattern migration 015 used for `important_dates`), and adds `retry_count`/`permanently_failed`/`sent_at`/`claimed_at` for real Resend delivery via `/api/thank-you/process`. The pre-existing `on_purchase_created` trigger (external-flow auto thank-you creation) is deliberately left untouched by migration 016 — it's already running in production with no verifiable definition from this repo, so it wasn't reconstructed. Migration 016 adds a genuinely new `on_order_confirmed_thank_you` trigger for the catalog flow (fires when a wishlist-originated order's `status` changes to `confirmed`; guarded against firing twice for the same order). Owner-participant RLS: `receiver_id = auth.uid()` or `buyer_id = auth.uid()` can `SELECT`; only the receiver can `INSERT` a `type = 'personal'` row (app-level ownership checks in `/api/thank-you/[id]/personal` are the primary enforcement, RLS is defense-in-depth). Managed via `/api/thank-you/process` (cron) and `/api/thank-you/[id]/personal`; read via `/api/gifts`.
...
### `important_dates`
The receiver's personal calendar of *other* people's occasions (Flow 1 reminders) — `person_name`, `occasion_type` (same enum values as `occasions.occasion_type`), `date`, `is_recurring` (default true), `linked_wishlist_id` (optional, FK to `wishlists`, set when the receiver pastes that person's shared wishlist link and it resolves via `gifvtme_get_shared_wishlist`). Owner-only RLS (`user_id = auth.uid()`). Existed live with no migration file backing it until migration 015 — that migration is the first one to formally own its schema (`CREATE TABLE IF NOT EXISTS` + defensive `ADD COLUMN IF NOT EXISTS` patches, matching how this repo handles other tables created outside its migration history). Managed via `/api/important-dates` and `/api/important-dates/[id]`, hard-deleted (no soft-archive) since it's a simple personal note list with no purchase/order history riding on it. Deleting a row cascades to every `reminders` row referencing it via `important_date_id` (migration 015 makes this FK's `ON DELETE CASCADE` explicit — see `reminders` below) so the delete is a single atomic statement and can't fail on a leftover *sent* reminder.

### `reminders`
Generic reminder queue. `reminder_type` (`occasion_owner`/`invitee`) plus exactly one of `important_date_id`, `invite_id`, or `occasion_id` set, enforced by a check constraint. `occasion_id` links owner reminders created for a user-created occasion so rescheduling or archiving one occasion only deletes that occasion's unsent reminders. `important_date_id`'s FK predated this repo's migration history with an unverifiable `ON DELETE` policy; migration 015 makes it explicit (`ON DELETE CASCADE`, re-added via a DO-block drop-if-exists + re-create, same pattern migration 005 used for the check constraint) so deleting an important date can't fail on a leftover *sent* reminder still referencing it. `channel` (`email`/`push` as a value — but `push` is never scheduled by any code path since `lib/reminders/constants.ts`'s `REMINDER_CHANNELS` is `email`-only; there's no push delivery infrastructure to ever dispatch one, so no reminder-scheduling function creates that dead-end row), `scheduled_at`, `sent` flag. Migration 015 adds `days_before` (14 or 3, persisted at scheduling time so the email builder doesn't need to recompute it), `retry_count` (increments on a failed Resend send), `permanently_failed` (set once `retry_count` reaches 5, per business rule — stops further retries on a dead address), `sent_at`, `advance_pending` (set alongside `sent=true` on a recurring important date's 3-day reminder; the cron retries just the important-date advance/reschedule step on later runs without re-sending the email if that follow-up step fails independently — see `/api/reminders`'s `retryPendingAdvancements`), and `claimed_at` (an atomic conditional-`UPDATE` lease the cron takes on a row before sending it, so two overlapping cron invocations can't both send the same reminder; a claim older than 10 minutes is treated as abandoned and re-claimable).
...
### `reviews`
Added by migration 020 (`17-REVIEWS.md`) — the first table in this repo's schema history that never existed live beforehand, so it's a plain `CREATE TABLE`, not the defensive `CREATE IF NOT EXISTS` pattern migrations 015/016/019 needed. Columns: `id`, `user_id` (FK to `public.users`, cascades on delete), `catalog_product_id` (Sanity `_id`, plain TEXT — no FK, since Sanity documents aren't in this database), `rating` (1–5, checked), `body` (optional, ≤1000 chars, checked), `created_at`, `updated_at` (auto-maintained by a new `update_reviews_updated_at` trigger). `one_review_per_user_per_product` UNIQUE constraint on `(user_id, catalog_product_id)` enforces business rule #14 at the DB level. No `order_item_id` column — an earlier draft of this note assumed one would be needed to gate verified purchases, but the actual gate is a runtime join (`isVerifiedPurchaser` in `lib/reviews/server.ts`, checking `orders`/`order_items` for a `delivered` order containing the product), primarily enforced in the `POST /api/reviews` route handler (it's what produces the friendly 403 message) and backstopped by the same join expressed directly in the INSERT/UPDATE RLS `WITH CHECK` clauses (see `API_ROUTES.md`) — so a client writing straight through PostgREST with a valid session, bypassing the route entirely, still can't insert or repoint a review at a product it wasn't delivered. Migration 020 also defines `update_updated_at_column()` itself — the spec assumed it already existed from migrations 001/002, but no migration file in this repo ever defined it or used an `updated_at` trigger before this one.
