...
Built from this spec (migration 020, `lib/reviews/`, `POST`/`GET /api/reviews`, `PATCH`/`DELETE /api/reviews/[id]`, `/reviews/new`, `components/review/`). The read side (`RatingBreakdown`, the review types, and a `getProductReviewsSummary`/`canUserReviewProduct` pair already wired into `ProductDetail`'s Reviews tab) was scaffolded in an earlier session ahead of this one, gracefully rendering empty/ineligible while the `reviews` table didn't exist yet — this pass built what that scaffolding was waiting on. The rest of this file is left as written except for the divergences below — see `ROADMAP.md`'s Reviews entry for the full list:

- **`update_updated_at_column()` didn't already exist.** The Database Changes section assumes it does "from migrations 001/002" — no migration file in this repo, 001/002 included, ever defines it or uses an `updated_at` trigger. Migration 020 defines it itself (`CREATE OR REPLACE`) rather than assuming, since guessing wrong here would fail the migration outright, not just risk a behavior mismatch.
- **No `order_item_id` column.** `DATABASE_SCHEMA.md`'s pre-migration note assumed one would be needed for verified-purchase gating; the actual gate is a runtime join (`isVerifiedPurchaser`) in the route handler, matching this spec's own Backend Logic and Permissions sections, which never mention such a column either — only its own outdated companion note did.
- **`POST /api/reviews`'s 409-on-`23505` is implemented verbatim**, including its own internal inconsistency: Edge Case #2 says a concurrent duplicate insert should resolve silently by updating the existing row, but the literal Backend Logic pseudocode returns 409 instead when that race is hit (only reachable when the initial existence check itself lost the race). Implemented as written rather than guessing which is correct — same call as migration 019's status-transition map.
- **`canLeaveReview` means "eligible and hasn't already reviewed," not just "eligible."** The UI Requirements section only states the purchase-eligibility condition, but the Manual QA section explicitly expects the CTA to disappear once a review exists and reappear after deleting it — eligibility alone can't produce that.
- **A direct visit to `/reviews/new` by an ineligible user renders an inline message, not the spec's toast-driven redirect.** The "Product not found" case (missing/unresolvable `product_id`) does redirect to `/shop`, but without the accompanying toast — no toast-via-redirect convention exists anywhere in this codebase to reuse, and inventing one for this single low-traffic edge (the CTA already gates the common path) wasn't worth it, the same call the `14-SEARCH.md` pass made about not inventing a one-off breadcrumb.
- **`isVerifiedPurchaser` has no unit test**, despite the Testing Requirements section asking for one. No test in this repo mocks a `SupabaseClient` — its predecessor, `canUserReviewProduct`, was never tested either. The rating-aggregate math was extracted into a pure `lib/reviews/aggregates.ts` instead, which is unit tested.
- **Pagination reuses the existing "Load more" button** rather than adding numbered pagination — `ReviewsList` now backs it with real `GET /api/reviews?page=` calls (10 per page, per FR9) instead of the pre-existing scaffolding's behavior of revealing more of an already-fetched, capped-at-50 list.
