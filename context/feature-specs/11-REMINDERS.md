...
## Overview
Two independent reminder flows sharing the same `reminders` table and delivery infrastructure. **Flow 1:** The receiver tracks other people's occasions and gets reminded before they arrive (e.g. "Mum's birthday in 3 days"). **Flow 2:** Invited givers opt in to be reminded about the receiver's occasion (e.g. "Sarah's birthday is in 14 days"). Both flows schedule two reminders per occasion: 14 days before and 3 days before. Delivery is via email (Resend) — push notification delivery remains deferred. `reminders.channel` supports `push` as a value, but no scheduling function actually creates `push` rows (`REMINDER_CHANNELS` in `lib/reminders/constants.ts` is `email`-only) since there's no push infrastructure anywhere in this codebase to ever deliver one — scheduling an undeliverable channel would just accumulate dead rows forever, worse so for a recurring important date rescheduling every year. A cron job (`POST /api/reminders`) processes the `reminders` table and sends due reminders.
...
3. On create: `createImportantDate` calls `scheduleImportantDateReminders(importantDateId, date)` → builds 2 candidate `reminders` rows (14-day and 3-day before `date`) with `reminder_type='occasion_owner'`, `important_date_id` set, `channel='email'`, `sent=false`, `days_before` set to `14`/`3`, but only inserts the rows whose computed `scheduled_at` is still in the future — a date less than 14 days out only gets the 3-day row, and a date less than 3 days out gets neither (see Edge Cases).
...
2. **User deletes an important date while a reminder is in the cron's due batch.** `DELETE /api/important-dates/[id]` is a single `DELETE` on `important_dates`; migration 015 makes `reminders.important_date_id`'s `ON DELETE CASCADE` explicit (its behavior previously predated this repo's migration history and wasn't verifiable from the code), so every reminder pointing at that date — sent or unsent — is removed atomically in the same statement, not via a separate app-side delete. If the cron already fetched a due reminder into memory before the cascade runs, its per-row claim (`claimReminder`, a conditional `UPDATE ... WHERE id = ...`) simply matches zero rows once the row is gone and the cron skips it — no orphaned row, no error, no retry needed. `buildReminderEmail` returning `null` for a missing join (and the cron deleting that row) remains the fallback for the other reminder sources (`occasion_id`, `invite_id`), which aren't covered by this specific cascade.
...
## Future Improvements
- Push notification delivery (requires native app or web push setup, e.g. FCM/APNs or web push subscriptions) — `reminders.channel` already supports a `push` value and the cron's plumbing would recognize it, but no scheduling function currently creates `push` rows (`REMINDER_CHANNELS` is `email`-only) since there's nowhere for one to be delivered to yet.
