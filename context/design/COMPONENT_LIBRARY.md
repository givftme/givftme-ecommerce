...
### `PriceDisplay`
Props: `price` (number, required), `compareAtPrice` (number, optional — renders strikethrough if present and greater than `price`), `size` (`sm`|`md`|`lg`), `isOnFlashSale` (boolean, optional — renders `price` in brand red instead of the default ink color; used by `ProductCard`/`ProductDetail` for active flash sales). Always renders in Naira via `formatPrice()` — never pass a currency code.
...
`checkout/CheckoutForm.tsx`, `checkout/AddressSelector.tsx`, `checkout/PaymentMethodSelector.tsx`, and `checkout/OrderSummaryPanel.tsx` implement the protected checkout details screen with react-hook-form + Zod validation, Nigerian state selection, future-ready saved-address prefill, Flutterwave payment preference, and responsive form/summary layout. `checkout/PriceChangeDialog.tsx` (`18-FLASH-SALES.md` Edge Case #2) is a blocking confirmation dialog shown when `POST /api/checkout` reports `price_changed: true` (a flash sale ended between page load and submit) — the order is already created at the correct server price by the time it appears; the dialog just stops the automatic redirect to the Flutterwave payment link until the buyer acknowledges the new price.
...
`flash-sale/FlashSaleBanner.tsx` (homepage hero banner), `flash-sale/FlashSaleNavbarStrip.tsx` (thin strip in `Navbar`, fed by a `PageWrapper`-level fetch so every public page gets it without per-page prop drilling), and `flash-sale/FlashSaleTimer.tsx` (countdown — `H:MM:SS` above 1hr, `MM:SS` amber under 1hr, `MM:SS` red with a GSAP pulse at ≤60s; pass `disableUrgencyColor` when placing it on a brand-red background, e.g. inside the banner/strip themselves, since the urgency red text would otherwise be invisible on brand-red) together implement `18-FLASH-SALES.md`. `app/flash-sale/page.tsx` (`revalidate = 30`) reuses `ProductExplorer` (the same grid/filter/sort/load-more component `/shop` uses) against `/api/flash-sale/products`, so it's no longer deferred.

`review/StarRating.tsx`, `review/ReviewCard.tsx`, `review/ReviewsList.tsx`, and `review/ReviewForm.tsx` implement Reviews (`17-REVIEWS.md`). `StarRating` is the interactive 5-star selector used by `ReviewForm` (large, GSAP scale-bump on click) — read-only star display elsewhere (the product page's rating summary link, `ReviewCard`) stays hand-rolled with plain lucide `Star` icons rather than reusing it, since their sizing/behavior needs differ enough that forcing a shared component would add props rather than remove code. `ReviewsList` is the Reviews tab's full content: `RatingBreakdown` (still in `product/`, see below) + review cards + a "Load more" button backed by `GET /api/reviews`'s pagination + the "Write a review" CTA (shown only when eligible and not already reviewed) + the delete-confirmation dialog. `ReviewCard` shows the reviewer avatar/name, relative date (`formatRelativeTime()` in `lib/utils.ts`), a 3-line-clamped body with "Read more", and Edit/Delete links visible only to the review's author (compared against a server-supplied `currentUserId`, threaded down from `app/product/[slug]/page.tsx` through `ProductDetail`). `ReviewForm` is the react-hook-form + Zod submission/edit form used by `/reviews/new` — the same POST endpoint handles both create and edit (the API auto-detects an existing review), so the form never needs to branch on mode beyond its own labels/pre-fill.

`product/RatingBreakdown.tsx` (not `review/`) — it already existed there, pre-built ahead of the rest of Reviews and wired into `ProductDetail`'s tab, so it stayed put rather than moving into the new `review/` folder for a purely cosmetic reorganization. It now animates its bars with GSAP on mount (`useGSAP`) — since `ReviewsList` (and this component with it) unmounts whenever the product page's Description tab is active, mounting naturally coincides with "the Reviews tab becoming visible" (spec Edge Case #6), no `IntersectionObserver` needed.
